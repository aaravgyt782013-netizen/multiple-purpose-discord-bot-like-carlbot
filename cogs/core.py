...
CATEGORIES = [
    ...
    ("Giveaways", "🎁", {"Giveaways", "AdvancedSafe"}),
...
class HelpView(discord.ui.View):
    ...
    def rebuild(self):
        ...
        if self.category:
            ...
            if pages > 1:
                ...
                self.add_item(back); self.add_item(nxt)
    ...
    def home_embed(self):
        ...
        for n, e, _ in CATEGORIES:
            embed.add_field(name=f"{e} {n}", value="Use the dropdown below.", inline=True)
    ...
    async def home_callback(self, interaction):
        self.category = None; self.page = 0; self.rebuild()
    ...
    async def next_callback(self, interaction):
        pages = max(1, math.ceil(len(self.entries()) / 8)); self.page = min(pages - 1, self.page + 1); self.rebuild()
        await interaction.response.edit_message(embed=self.category_embed(), view=self)
...
class Core(commands.Cog):
    ...
    async def about(self, ctx):
        embed = discord.Embed(title="💠 LightCore", description="All-in-one Discord moderation, community, leveling, tickets, music and utility bot.", color=discord.Color.blurple())
        links = []
        if os.getenv("SUPPORT_SERVER_URL"): links.append(f"[Support Server]({os.getenv('SUPPORT_SERVER_URL')})")
        if os.getenv("PRIVACY_POLICY_URL"): links.append(f"[Privacy Policy]({os.getenv('PRIVACY_POLICY_URL')})")
        if os.getenv("TERMS_URL"): links.append(f"[Terms of Service]({os.getenv('TERMS_URL')})")
        if os.getenv("INVITE_URL"): links.append(f"[Invite LightCore]({os.getenv('INVITE_URL')})")
