...
class MemberStats(commands.Cog):
    ...
    @commands.hybrid_command(name="memberstats", description="Show member growth and activity statistics.")
    ...
    async def memberstats(self, ctx):
        ...
        with connect() as db:
            ...
            active = db.execute("SELECT user_id, COUNT(*) c FROM member_events WHERE guild_id=? AND event_type='join' GROUP BY user_id ORDER BY c DESC LIMIT 5", (ctx.guild.id,)).fetchall()
        retention = round((joins - leaves) / joins * 100, 1) if joins else 0
        lines = []
        for row in active:
            user = ctx.guild.get_member(row[0])
            lines.append(f"{user.mention if user else row[0]} — {row[1]} recorded joins")
        ...
        embed.add_field(name="Top recorded join activity", value="\n".join(lines) if lines else "No data yet.", inline=False)
        embed.set_footer(text="LightCore • Member Stats")
        await ctx.send(embed=embed)

    @commands.hybrid_command(name="growth", description="Show recent daily join/leave growth data.")
    @commands.has_permissions(manage_guild=True)
    async def growth(self, ctx, days: commands.Range[int, 1, 30] = 14):
        start = datetime.now(timezone.utc) - timedelta(days=days)
        with connect() as db:
            rows = db.execute("SELECT substr(created_at,1,10) day, event_type, COUNT(*) c FROM member_events WHERE guild_id=? AND created_at>=? GROUP BY day,event_type ORDER BY day", (ctx.guild.id, start.strftime("%Y-%m-%d %H:%M:%S"))).fetchall()
        by_day = {}
        for row in rows: by_day.setdefault(row[0], {})[row[1]] = row[2]
        text = "\n".join(f"`{day}`  +{data.get('join',0)} / -{data.get('leave',0)}" for day, data in by_day.items()) or "No recorded events in this period."
        await ctx.send(embed=discord.Embed(title=f"Member Growth • Last {days} days", description=text, color=discord.Color.blurple()))
