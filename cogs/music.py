...
class Music(commands.Cog):
    ...
    def _set_player_autoplay(self, player, enabled):
        if wavelink is None or not isinstance(player, wavelink.Player): return
        player.autoplay = wavelink.AutoPlayMode.enabled if enabled else wavelink.AutoPlayMode.disabled
        if not enabled:
            try: player.auto_queue.clear()
            except Exception: pass

    @property
    def node(self):
        if wavelink is None: return None
        try:
    ...
    def node_error(self):
        if wavelink is None: return "Wavelink is not installed."
        try:
            node = wavelink.Pool.get_node()
            return f"No Lavalink node is connected (current node status: `{getattr(node, 'status', 'unknown')}`)."
        except Exception: return "No Lavalink node is connected."

    async def _player(self, ctx):
        if not ctx.guild: return None
        if not ctx.author.voice or not ctx.author.voice.channel:
            await ctx.send("🎧 Join a voice channel first."); return None
        if self.node is None:
            await ctx.send(f"❌ {self.node_error()}"); return None
        ...
        if player is None:
            try:
                ...
                await ctx.send("❌ I need View Channel, Connect and Speak permissions."); return None
            except Exception as exc:
                log.exception("Voice connection failed")
                await ctx.send(f"❌ Voice connection failed: `{type(exc).__name__}`."); return None
        elif isinstance(player, wavelink.Player) and player.channel != channel:
            try: await player.move_to(channel)
            except Exception as exc:
                await ctx.send(f"❌ I could not move to your voice channel: `{type(exc).__name__}`."); return None
        return player
    ...
    def _embed(self, guild_id):
        guild = self.bot.get_guild(guild_id); player = guild.voice_client if guild else None
        ...
        embed = discord.Embed(title="🎵 Now Playing", description=f"[{track.title}]({url})\n\nDuration: {self._duration(track.length)} • Requested By: {by}", color=discord.Color.blurple())
    ...
    async def _update_panel(self, guild_id, message=None):
        ...
        try: await message.edit(embed=self._embed(guild_id), view=MusicPanel(self, guild_id))
        except (discord.NotFound, discord.HTTPException): self.panel_messages.pop(guild_id, None)

    async def _send_or_update_panel(self, ctx):
        if self.panel_messages.get(ctx.guild.id):
            await self._update_panel(ctx.guild.id); return
        self.panel_messages[ctx.guild.id] = await ctx.send(embed=self._embed(ctx.guild.id), view=MusicPanel(self, ctx.guild.id))

    @commands.hybrid_command(name="join", description="Join your voice channel.")
    async def join(self, ctx):
        if await self._player(ctx): await ctx.send("🎵 Connected through Lavalink.")
    ...
    async def play(self, ctx, *, query: str):
        ...
        try: tracks = await wavelink.Playable.search(query)
            ...
            log.exception("Search failed"); return await ctx.send(f"❌ Search failed: `{type(exc).__name__}`.")
        if not tracks: return await ctx.send("🔎 No playable track was found.")
        track = tracks[0]; self.track_requesters[id(track)] = ctx.author.id
        if self.autoplay[ctx.guild.id]:
            self._save_autoplay(ctx.guild.id, False); self._set_player_autoplay(player, False)
        if player.current:
    ...
    async def autoplay_command(self, ctx):
        enabled = not self.autoplay[ctx.guild.id]; self._save_autoplay(ctx.guild.id, enabled)
        player = ctx.voice_client
        if isinstance(player, wavelink.Player): self._set_player_autoplay(player, enabled)
    ...
    async def pause(self, ctx):
        player = ctx.voice_client
        if not isinstance(player, wavelink.Player) or not player.current: return await ctx.send("❌ Nothing is currently playing.")
        await player.pause(not player.paused); await self._update_panel(ctx.guild.id)
    ...
    async def stop(self, ctx):
        player = ctx.voice_client
        if not isinstance(player, wavelink.Player): return await ctx.send("❌ Music is not connected.")
        self.queues[ctx.guild.id].clear(); self.loop_modes[ctx.guild.id] = "off"; self._save_autoplay(ctx.guild.id, False); self._set_player_autoplay(player, False); await player.stop(); await self._update_panel(ctx.guild.id)
    ...
    async def volume(self, ctx, value: commands.Range[int, 0, 100]):
        player = ctx.voice_client
        if not isinstance(player, wavelink.Player): return await ctx.send("❌ Music is not connected.")
        await player.set_volume(value); await self._update_panel(ctx.guild.id)

    @commands.hybrid_command(name="nowplaying", aliases=["np"], description="Show the current track.")
    async def nowplaying(self, ctx):
        if not isinstance(ctx.voice_client, wavelink.Player) or not ctx.voice_client.current: return await ctx.send("❌ Nothing is currently playing.")
        await self._send_or_update_panel(ctx)

    @commands.hybrid_command(name="queue", aliases=["q"], description="Show the music queue.")
    async def queue(self, ctx):
        q = self.queues[ctx.guild.id]
        if not q: return await ctx.send("📜 The music queue is empty.")
    ...
    async def leave(self, ctx):
        ...
        if isinstance(player, wavelink.Player):
            self.queues[ctx.guild.id].clear(); await player.disconnect(); self.panel_messages.pop(ctx.guild.id, None); await ctx.send("👋 Left the voice channel.")
        else: await ctx.send("I am not connected to voice.")

    @commands.Cog.listener()
    async def on_wavelink_track_start(self, payload):
        if payload.player and payload.track:
            gid = payload.player.guild.id; self.requesters[gid] = self.track_requesters.get(id(payload.track), self.requesters.get(gid)); await self._update_panel(gid)
    ...
    async def on_wavelink_track_end(self, payload):
        ...
        gid = player.guild.id
        if self.loop_modes[gid] == "queue" and payload.track: self.queues[gid].append(payload.track)
        if self.queues[gid]:
            track = self.queues[gid].popleft(); self.requesters[gid] = self.track_requesters.get(id(track), self.requesters.get(gid)); await player.play(track, populate=self.autoplay[gid], max_populate=5)
        await self._update_panel(gid)
...
class MusicPanel(discord.ui.View):
    def __init__(self, cog, guild_id):
        ...
        for label, emoji, action, style, row in controls:
            b = discord.ui.Button(label=label, emoji=emoji, style=style, custom_id=f"lightcore:music:{guild_id}:{action}", row=row); b.callback = self._callback(action); self.add_item(b)

    def _callback(self, action):
        async def callback(interaction):
            guild = interaction.guild; player = guild.voice_client if guild else None
            if not isinstance(player, wavelink.Player): return await interaction.response.send_message("❌ Music is not connected.", ephemeral=True)
            if action == "pause": await player.pause(not player.paused)
            elif action == "skip": await player.skip()
            elif action == "stop":
                self.cog.queues[self.guild_id].clear(); self.cog.loop_modes[self.guild_id] = "off"; self.cog._save_autoplay(self.guild_id, False); self.cog._set_player_autoplay(player, False); await player.stop()
            elif action == "shuffle": random.shuffle(self.cog.queues[self.guild_id])
            elif action == "loop":
                modes = ["off", "track", "queue"]; self.cog.loop_modes[self.guild_id] = modes[(modes.index(self.cog.loop_modes[self.guild_id]) + 1) % len(modes)]
            elif action == "queue":
