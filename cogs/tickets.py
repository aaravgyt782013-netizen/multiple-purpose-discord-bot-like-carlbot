...
class TicketCategoryView(discord.ui.View):
    def __init__(self, cog, guild_id):
        ...
        options = [discord.SelectOption(label=item["name"][:100], value=str(index), emoji=item.get("emoji") or "🎫", description="Open a ticket in this category.") for index, item in enumerate(config["categories"][:25])]
        if not options:
    ...
    async def create_ticket(self, interaction):
        ...
        index = int(interaction.data["values"][0]); config = load_config(interaction.guild.id)
        if index >= len(config["categories"]):
            return await interaction.response.send_message("❌ That category no longer exists. Re-publish the panel.", ephemeral=True)
        ...
        embed.add_field(name="Ticket", value=f"`#{ticket_id}`", inline=True); embed.add_field(name="Category", value=item["name"], inline=True)
        if config.get("claim_required"):
            view = TicketClaimView(interaction.guild.id, ticket_id, item.get("role_id")); view.ticket_id = ticket_id
...
class TicketEditor(discord.ui.View):
    def __init__(self, cog, guild_id):
        super().__init__(timeout=300); self.cog = cog; self.guild_id = guild_id; self.message = None; self.selected_category = 0; self.rebuild()
    ...
    def rebuild(self):
        self.clear_items()
        buttons = [("Panel Text", "📝", "text", discord.ButtonStyle.primary), ("Color", "🎨", "color", discord.ButtonStyle.secondary), ("Panel Channel", "📍", "channel", discord.ButtonStyle.secondary), ("Claim", "🙋", "claim", discord.ButtonStyle.primary), ("Categories", "🗂️", "categories", discord.ButtonStyle.secondary), ("Publish Panel", "📢", "publish", discord.ButtonStyle.success)]
        for i, (label, emoji, action, style) in enumerate(buttons):
            button = discord.ui.Button(label=label, emoji=emoji, style=style, custom_id=f"lightcore:ticketsetup:{self.guild_id}:{action}", row=i // 4); button.callback = self.callback(action); self.add_item(button)

    async def interaction_check(self, interaction):
        if not interaction.user.guild_permissions.manage_channels:
            await interaction.response.send_message("❌ You need Manage Channels to edit ticket setup.", ephemeral=True); return False
        return True
    ...
    def callback(self, action):
        async def inner(interaction):
            ...
            if action == "channel": return await interaction.response.send_message("Choose the channel where the final ticket panel should be posted.", view=PanelChannelView(self), ephemeral=True)
            if action == "claim":
                cfg = self.config(); cfg["claim_required"] = not cfg.get("claim_required", False); save_config(self.guild_id, cfg); return await interaction.response.edit_message(embed=self.embed(), view=self)
...
class TicketTextModal(discord.ui.Modal, title="Edit Ticket Panel Text"):
    ...
    def __init__(self, editor):
        super().__init__(); self.editor = editor; cfg = editor.config(); self.title_input.default = cfg.get("title", ""); self.description_input.default = cfg.get("description", "")
    async def on_submit(self, interaction):
        cfg = self.editor.config(); cfg["title"] = str(self.title_input); cfg["description"] = str(self.description_input); save_config(self.editor.guild_id, cfg); await interaction.response.edit_message(embed=self.editor.embed(), view=self.editor)
...
class TicketColorModal(discord.ui.Modal, title="Edit Ticket Panel Color"):
    ...
    async def on_submit(self, interaction):
        ...
        cfg = self.editor.config(); cfg["color"] = value; save_config(self.editor.guild_id, cfg); await interaction.response.edit_message(embed=self.editor.embed(), view=self.editor)
...
class PanelChannelView(discord.ui.View):
    ...
    async def select(self, interaction):
        channel_id = interaction.data.get("values", [None])[0]; cfg = self.editor.config(); cfg["panel_channel_id"] = int(channel_id); save_config(self.editor.guild_id, cfg); await interaction.response.send_message("✅ Panel channel saved.", ephemeral=True)


class CategoryManagerView(discord.ui.View):
    def __init__(self, editor):
        super().__init__(timeout=300); self.editor = editor; self.rebuild()
    def config(self): return self.editor.config()
    def embed(self):
        cfg = self.config(); embed = discord.Embed(title="🗂️ Ticket Categories", description="Select a category to edit, or add/remove/rename one. Each category has its own emoji, support role and Discord parent category.", color=discord.Color(cfg.get("color", 0x5865F2)))
        lines = []
        for i, item in enumerate(cfg.get("categories", []), 1):
            role_text = f"<@&{item['role_id']}>" if item.get("role_id") else "none"; parent_text = f"<#{item['category_id']}>" if item.get("category_id") else "none"
            lines.append(f"**{i}.** {item.get('emoji','🎫')} {item.get('name','Unnamed')} • role: {role_text} • parent: {parent_text}")
        embed.add_field(name="Configured", value="\n".join(lines) or "None", inline=False); return embed
    def rebuild(self):
        ...
        for label, emoji, action, style in [("Add", "➕", "add", discord.ButtonStyle.success), ("Remove", "➖", "remove", discord.ButtonStyle.danger), ("Rename", "✏️", "rename", discord.ButtonStyle.primary), ("Back", "↩️", "back", discord.ButtonStyle.secondary)]:
            button = discord.ui.Button(label=label, emoji=emoji, style=style, custom_id=f"lightcore:ticketsetup:cat:{self.editor.guild_id}:{action}"); button.callback = self.action(action); self.add_item(button)
    async def select(self, interaction):
    ...
    def action(self, action):
        async def inner(interaction):
            ...
            if action == "remove": cfg["categories"].pop(index); save_config(self.editor.guild_id, cfg); self.rebuild(); return await interaction.response.edit_message(embed=self.embed(), view=self)
            if action == "rename": return await interaction.response.send_modal(RenameCategoryModal(self.editor, index))
        return inner
...
class CategoryModal(discord.ui.Modal, title="Add Ticket Category"):
    ...
    async def on_submit(self, interaction):
        try: role_id = int(str(self.support_role_id)) if str(self.support_role_id).strip() else None; parent_id = int(str(self.parent_category_id)) if str(self.parent_category_id).strip() else None
        except ValueError: return await interaction.response.send_message("❌ Role/category IDs must be numbers.", ephemeral=True)
        cfg = self.editor.config(); cfg["categories"].append({"name": str(self.name).strip(), "emoji": str(self.emoji).strip() or "🎫", "role_id": role_id, "category_id": parent_id}); cfg["categories"] = cfg["categories"][:25]; save_config(self.editor.guild_id, cfg); await interaction.response.send_message("✅ Category saved. Re-run `.ticketsetup` to continue editing.", ephemeral=True)
...
class RenameCategoryModal(discord.ui.Modal, title="Rename Ticket Category"):
    ...
    def __init__(self, editor, index): super().__init__(); self.editor = editor; self.index = index; self.name.default = editor.config()["categories"][index]["name"]
    async def on_submit(self, interaction):
        cfg = self.editor.config(); cfg["categories"][self.index]["name"] = str(self.name).strip(); save_config(self.editor.guild_id, cfg); await interaction.response.send_message("✅ Category renamed. Re-run `.ticketsetup` to see the updated list.", ephemeral=True)
...
class Tickets(commands.Cog):
    ...
    async def on_ready(self):
        for guild in self.bot.guilds:
            ...
            with connect() as db:
                rows = db.execute("SELECT ticket_id FROM ticket_meta WHERE ticket_id IN (SELECT id FROM tickets WHERE guild_id=? AND status='open')", (guild.id,)).fetchall()
            ...
            for row in rows:
                ...
                if cfg.get("claim_required"): self.bot.add_view(TicketClaimView(guild.id, ticket_id, role_id))
    ...
    async def publish_panel(self, source, guild_id):
        ...
        if not isinstance(channel, discord.TextChannel):
            if hasattr(source, "response"): return await source.response.send_message("❌ Choose a text channel first with **Panel Channel**.", ephemeral=True)
            return await source.send("❌ Choose a text channel first with **Panel Channel**.")
        if not cfg.get("categories"):
            if hasattr(source, "response"): return await source.response.send_message("❌ Add at least one ticket category first.", ephemeral=True)
            return await source.send("❌ Add at least one ticket category first.")
        ...
        message = await channel.send(embed=embed, view=TicketCategoryView(self, guild_id)); cfg["published_message_id"] = message.id; save_config(guild_id, cfg)
        if hasattr(source, "response"): return await source.response.send_message(f"✅ Ticket panel published in {channel.mention}. Re-run `.ticketsetup` any time to edit it.", ephemeral=True)
    ...
    async def close(self, ctx):
        ...
        try:
            async for message in ctx.channel.history(limit=500, oldest_first=True):
                stamp = message.created_at.strftime("%Y-%m-%d %H:%M:%S UTC"); lines.append(f"[{stamp}] {message.author} ({message.author.id}): {message.content.replace(chr(10), ' ')}")
        except discord.HTTPException: pass
        transcript = "\n".join(lines) or "No messages captured."
        with connect() as db:
            db.execute("UPDATE tickets SET status='closed', closed_at=CURRENT_TIMESTAMP WHERE id=?", (row["id"],)); db.execute("INSERT INTO ticket_transcripts(ticket_id,transcript) VALUES(?,?)", (row["id"], transcript[:50000]))
