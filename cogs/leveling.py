...
class Leveling(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if not get_setting(guild_id, "level_enabled"):
            return
        ...
        lo = max(1, int(get_setting(guild_id, "level_xp_min") or 15))
        hi = max(lo, int(get_setting(guild_id, "level_xp_max") or 25))
        ...
        with connect() as db:
            ...
            while xp >= self.needed(level):
                xp -= self.needed(level)
                level += 1
                leveled = True
            db.execute(
                "INSERT OR REPLACE INTO xp(guild_id,user_id,xp,level) VALUES(?,?,?,?)",
                (guild_id, user_id, xp, level),
            )
            reward = db.execute(
                "SELECT role_id FROM level_rewards WHERE guild_id=? AND level=?",
                (guild_id, level),
            ).fetchone() if leveled else None

        if leveled:
            template = get_setting(guild_id, "level_message") or "🎉 {user} reached **Level {level}**!"
            await message.channel.send(
                template.replace("{user}", message.author.mention).replace("{level}", str(level))
            )
            if reward:
                role = message.guild.get_role(reward["role_id"])
                if role:
                    try:
                        await message.author.add_roles(role, reason="LightCore level reward")
    ...
    async def rank(self, ctx, member: discord.Member = None):
        ...
        with connect() as db:
            ...
            xp, level = (row["xp"], row["level"]) if row else (0, 0)
            position = db.execute(
                "SELECT COUNT(*) FROM xp WHERE guild_id=? AND (level>? OR (level=? AND xp>?))",
                (ctx.guild.id, level, level, xp),
            ).fetchone()[0] + 1
    ...
    async def levelconfig(self, ctx):
        await ctx.send("Use `.levelconfig rate`, `.levelconfig cooldown`, `.levelconfig message`, `.levelconfig reward`, or `.levelconfig enable`.")

    @levelconfig.command(name="rate")
    async def rate(self, ctx, minimum: int = 8, maximum: int = 15):
        set_setting(ctx.guild.id, "level_xp_min", max(1, minimum))
        set_setting(ctx.guild.id, "level_xp_max", max(max(1, minimum), maximum))
    ...
    async def cooldown(self, ctx, seconds: commands.Range[int, 0, 3600] = 60):
        set_setting(ctx.guild.id, "level_cooldown", seconds)
        await ctx.send(f"✅ XP cooldown set to **{seconds}s**.")

    @levelconfig.command(name="message")
    async def message(self, ctx, *, text):
        set_setting(ctx.guild.id, "level_message", text)
    ...
    async def rank_card(self, member, xp, level, position):
        ...
        image.paste(avatar, (50, 55), mask)
        font = ImageFont.load_default(size=30)
        small = ImageFont.load_default(size=20)
        draw.text((240, 55), member.display_name[:24], fill="white", font=font)
        draw.text((240, 100), f"LEVEL {level}  •  RANK #{position}", fill=(180, 180, 195), font=small)
        needed = self.needed(level)
        draw.text((240, 140), f"XP  {xp:,} / {needed:,}", fill="white", font=small)
        draw.rounded_rectangle((240, 185, 840, 212), radius=10, fill=(50, 50, 60))
        draw.rounded_rectangle((240, 185, 240 + int(600 * min(1, xp / needed)), 212), radius=10, fill=(115, 125, 255))
