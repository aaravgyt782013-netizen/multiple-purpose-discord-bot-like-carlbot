...
class Utility(commands.Cog):
    ...
    async def cog_load(self):
        ...
        if music:
            async def fresh_panel(ctx):
                try:
                    msg=await ctx.send(embed=music._embed(ctx.guild.id),view=music_panel(music,ctx.guild.id))
                    music.panel_messages[ctx.guild.id]=msg
            ...
            if old:
                async def nowplaying(command_ctx):
                    player=command_ctx.voice_client
                    if not getattr(player,"current",None):return await command_ctx.send("❌ Nothing is currently playing.")
                    await fresh_panel(command_ctx)
    ...
    async def uptime(self,ctx):
        ...
        parts.append(f"{seconds:02d}s"); started=getattr(self.bot,"started_at_wall",None)
        embed=discord.Embed(title="⏱️ LightCore • Uptime",description=f"LightCore has been online for **{' '.join(parts)}**.",color=discord.Color.blurple())
        if started:embed.add_field(name="Started",value=discord.utils.format_dt(started,"F"),inline=False)
        embed.set_footer(text="LightCore • Utility");await ctx.send(embed=embed)
    @commands.hybrid_command(name="botinfo",description="Show LightCore version, library, server and shard information.")
    async def botinfo(self,ctx):
        users=sum(g.member_count or 0 for g in self.bot.guilds); shards=self.bot.shard_count or 1; sid=ctx.guild.shard_id if ctx.guild else 0
    ...
    async def avatar(self,ctx,member:discord.Member=None):
        member=member or ctx.author; e=discord.Embed(title=f"🖼️ {member.display_name}'s Avatar",color=discord.Color.blurple()); e.set_image(url=member.display_avatar.url); await ctx.send(embed=e)
...
class music_panel(discord.ui.View):
    ...
    def cb(self,action):
        async def callback(interaction):
            ...
            await interaction.response.edit_message(embed=self.cog._embed(self.guild_id),view=music_panel(self.cog,self.guild_id))
        return callback
