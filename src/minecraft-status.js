...
function saveMonitors(value) {
  fs.writeFileSync(MONITORS_FILE, JSON.stringify(value, null, 2));
}
...
function parseAddress(input) {
  ...
  if (value.startsWith("[")) {
    ...
    const port = value.slice(end + 1).startsWith(":")
      ? Number(value.slice(end + 2))
      : 25565;
...
async function resolvePublicNetwork(host, port) {
  ...
  let ip = null;
  try {
    ip = (await dns.lookup(targetHost, { family: 4 })).address;
  } catch {}

  return { targetHost, targetPort, ip, srv };
}

async function pingJavaServer(host, port) {
  return mc.ping({ host, port });
}
...
async function getStatus(address) {
  ...
  try {
    const result = await pingJavaServer(network.targetHost, network.targetPort);
    ...
    const sample = Array.isArray(result.players?.sample)
      ? result.players.sample.map(p => typeof p === "string" ? p : p?.name).filter(Boolean)
      : [];
    ...
    return {
      ...
      publicIp: network.ip,
      srv: network.srv ? \`${network.srv.host}:${network.srv.port}\` : null,
      motd: cleanMotd(result.description),
      version: version.name || "Unknown",
      protocol: version.protocol ?? "N/A",
      onlinePlayers: result.players?.online ?? 0,
      maxPlayers: result.players?.max ?? 0,
      players: sample,
      latency,
      proxy: detectProxy(version.name, result.description),
      error: null
    };
  } catch (error) {
    return {
...
function detectProxy(version, description) {
  const text = cleanMotd(description) + " " + String(version || "");
  const signatures = [];
  for (const name of ["velocity", "bungeecord", "waterfall", "travertine"]) {
    if (text.toLowerCase().includes(name)) signatures.push(name);
  }
  return signatures;
}
...
function statusEmbed(status, address) {
  ...
  if (status.online) {
    ...
    embed.addFields(
      ...
      { name: "Version", value: shorten(\`\${status.version} (protocol \${status.protocol})\`, 100), inline: true },
      { name: "Players", value: \`\${status.onlinePlayers} / \${status.maxPlayers}\`, inline: true },
      { name: "Latency", value: status.latency == null ? "N/A" : \`\${status.latency} ms\`, inline: true },
      { name: "Numeric IP", value: status.publicIp ? \\\`\${status.publicIp}:\${status.inputPort}\\\` : "Not resolved", inline: true },
      { name: "SRV", value: status.srv ? \\\`\${status.srv}\\\` : "N/A", inline: true }
    );

    if (status.proxy.length) {
      embed.addFields({
        name: "Proxy / Velocity Check",
        value: \`**Proxy detected** — signatures: \${status.proxy.join(", ")}\`
      });
    }

    embed.addFields({
      name: "Player Sample",
      value: status.players.length
        ? shorten(status.players.slice(0, 24).join(", "), 900)
        : "The server did not provide a player sample."
    });
  } else {
  ...
  embed.setFooter({
    text: "LightCore Minecraft Status • Public DNS/IP only; private proxy backends are not exposed"
  });
...
function actionRow(address) {
  ...
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId(\`mc-refresh:\${encoded}\`)
      .setLabel("Refresh")
      .setEmoji("🔄")
      .setStyle(ButtonStyle.Primary),
    new ButtonBuilder()
      .setCustomId(\`mc-players:\${encoded}\`)
      .setLabel("Players")
      .setEmoji("👥")
...
const commands = [
  new SlashCommandBuilder()
    .setName("mcstatus")
    .setDescription("Check a Minecraft Java server.")
    .addStringOption(o => o.setName("server").setDescription("Example: play.example.com:25565").setRequired(true)),
  new SlashCommandBuilder()
    .setName("mcplayers")
    .setDescription("Show the player sample returned by a Minecraft server.")
    .addStringOption(o => o.setName("server").setDescription("Minecraft server address").setRequired(true)),
  new SlashCommandBuilder()
    .setName("mcip")
    .setDescription("Show the public numeric IP and SRV target.")
    .addStringOption(o => o.setName("server").setDescription("Minecraft server hostname").setRequired(true)),
  new SlashCommandBuilder()
    .setName("mcmonitor")
    .setDescription("Create an automatic Minecraft status monitor in this channel.")
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild)
    .addStringOption(o => o.setName("server").setDescription("Minecraft server address").setRequired(true)),
  new SlashCommandBuilder()
    .setName("mcunmonitor")
    .setDescription("Stop a Minecraft status monitor.")
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild)
    .addStringOption(o => o.setName("server").setDescription("Minecraft server address").setRequired(true)),
  new SlashCommandBuilder()
    .setName("mcmonitors")
    .setDescription("List Minecraft monitors in this server.")
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild)
].map(x => x.toJSON());
...
async function handleCommand(interaction) {
  ...
  if (name === "mcplayers") {
    ...
    const players = status.players.length
      ? status.players.map((p, i) => \`\${i + 1}. \${p}\`).join("\\n")
      : "No player sample was provided by the server.";
    await interaction.editReply({
      embeds: [
        new EmbedBuilder()
          .setTitle(\`Online Player Sample — \${server}\`)
          .setColor(status.online ? 0x2ecc71 : 0xe74c3c)
          .setDescription(shorten(players, 3900))
          .setFooter({ text: \`\${status.onlinePlayers} / \${status.maxPlayers} online • Server-provided sample\` })
      ]
    });
    return true;
  }
  ...
  if (name === "mcip") {
    ...
    await interaction.editReply({
      embeds: [
        new EmbedBuilder()
          .setTitle(\`Network Details — \${server}\`)
          .setColor(status.online ? 0x2ecc71 : 0xe74c3c)
          .addFields(
            { name: "Status", value: status.online ? "🟢 Online" : "🔴 Offline", inline: true },
            { name: "Host", value: \\\`\${status.inputHost}\\\`, inline: true },
            { name: "Port", value: String(status.inputPort), inline: true },
            { name: "Public IP", value: status.publicIp ? \\\`\${status.publicIp}\\\` : "Not resolved", inline: true },
            { name: "SRV Target", value: status.srv ? \\\`\${status.srv}\\\` : "N/A", inline: false }
          )
  ...
  if (name === "mcmonitor") {
    ...
    const message = await interaction.channel.send({
      embeds: [statusEmbed(status, server)],
      components: [actionRow(server)]
    });

    const monitors = readMonitors().filter(
      x => !(x.guildId === interaction.guildId && x.server.toLowerCase() === server.toLowerCase())
    );
    monitors.push({
  ...
  if (name === "mcunmonitor") {
    ...
    await interaction.reply({
      content: after.length < before.length ? \`🛑 Stopped monitoring \\\`\${server}\\\`.\` : \`No monitor found for \\\`\${server}\\\`.\`,
      ephemeral: true
    });
    return true;
  }
  ...
  if (name === "mcmonitors") {
    ...
    const description = monitors.length
      ? monitors.map((x, i) => \`\${i + 1}. \\\`\${x.server}\\\` → <#\${x.channelId}>\`).join("\\n")
      : "No Minecraft monitors are configured.";
    await interaction.reply({
      embeds: [new EmbedBuilder().setTitle("Active Minecraft Monitors").setColor(0x5865f2).setDescription(description)],
      ephemeral: true
    });
    return true;
  }
...
async function handleButton(interaction) {
  ...
  const server = decodeURIComponent(encoded || "");
  const status = await getStatus(server);
...
async function updateMonitor(client, monitor) {
  try {
    ...
    await message.edit({
      embeds: [statusEmbed(status, monitor.server)],
      components: [actionRow(monitor.server)]
    });
  } catch (error) {
    console.error(\`Minecraft monitor \${monitor.server}: \${error.message}\`);
  }
}
...
function startMonitor(client) {
  const tick = async () => {
    ...
    for (const monitor of monitors) await updateMonitor(client, monitor);
  };
