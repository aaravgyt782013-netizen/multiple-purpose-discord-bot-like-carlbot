...
module.exports=function(client,prefix='.'){
 const P=prefix||'.';
 const groups={
  home:['help','ping','uptime','botinfo','invite','serverinfo','membercount','userinfo','avatar','roleinfo','channelinfo','servericon'],
 ...
 function help(cat='home'){
  ...
  if(cat==='home')return {embeds:[new EmbedBuilder().setTitle(`${em[Math.floor(Date.now()/5000)%em.length]} LIGHTCORE • HELP CENTER`).setDescription(`> **Prefix:** \`${P}\`\n> **Categories:** **${Object.keys(groups).length}**\n\n${Object.keys(groups).map((k,i)=>`${em[i%em.length]} **${names[k]}**\n   └─ \`${P}help ${k}\``).join('\n')}\n\n> 📌 Commands are displayed one-per-line for a clean, ordered layout.`).setColor([0x5865f2,0x7c3aed,0x2563eb,0x06b6d4][Math.floor(Date.now()/5000)%4]).setTimestamp()],components:[new ActionRowBuilder().addComponents(new StringSelectMenuBuilder().setCustomId('lc_help_ordered').setPlaceholder('📂 Select a category').addOptions(options.slice(0,25)))]};
