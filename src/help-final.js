...
module.exports=function(client,prefix='.'){
 const P=prefix||'.';
 const groups={
  home:['help','ping','botinfo','invite','serverinfo','userinfo','avatar','membercount'],
  ...
  security:['automod','antiraid','antinuke','security','filter','verify'],
  logging:['logsetup','setlog','logdisable','logsettings','logevents','logtest'],
  music:['play','p','join','connect','leave','disconnect','pause','resume','skip','next','stop','queue','q','nowplaying','np','volume','vol','loop','repeat','shuffle','remove','move','clearqueue','musichelp'],
  tickets:['ticket','open','ticket-panel','ticket-setup','close','claim','unclaim'],
  giveaways:['giveaway','gcreate'],
  fun:['8ball','coinflip','roll','choose','ship','rate','owo','owofy','owocify','cat','dog','joke'],
  ...
  counters:['counter','counter-setup','counter-update','counter-disable'],
  notifications:['notify','notify-add','notify-list','notify-remove','notify-test'],
 ...
 const emoji={spark:'✨',arrow:'➜',home:'🏠'};
 function help(g,cat='home'){
  ...
  if(cat==='home')return {embeds:[new EmbedBuilder().setTitle('✨ LIGHTCORE • HELP CENTER').setDescription(`> ${emoji.spark} **Prefix:** \`${P}\`\n> **Command groups:** **${Object.keys(groups).length}**\n\n${Object.keys(groups).map(k=>`${names[k]}  •  \`${P}help ${k}\``).join('\n')}\n\n> ${emoji.arrow} Select a category below.`).setColor(0x5865f2).setTimestamp()],components:[new ActionRowBuilder().addComponents(menu)]};
  const list=groups[cat].map(x=>`\`${P}${x}\``).join('  •  ');return {embeds:[new EmbedBuilder().setTitle(`${names[cat]} • COMMANDS`).setDescription(`> ✦ **${groups[cat].length}** commands\n\n${list}\n\n> ${emoji.arrow} Use \`${P}help\` to return to the command center.`).setColor(0x5865f2).setTimestamp()],components:[new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('lc_help_final_v2_home').setLabel('Help Home').setEmoji('🏠').setStyle(ButtonStyle.Secondary))]};
 }
 const old=client.listeners('messageCreate').at(-1);if(old){client.removeListener('messageCreate',old);client.on('messageCreate',async m=>{try{if(m.author.bot||!m.guild)return;if(!m.content.startsWith(P))return old(m);const a=m.content.slice(P.length).trim().split(/\s+/),c=(a.shift()||'').toLowerCase();if(c==='help'||c==='h'||c==='musichelp'||c==='music')return m.reply(help(m.guild,(c==='musichelp'||c==='music')?'music':(a[0]||'home').toLowerCase()));return old(m);}catch(e){console.error('[HELP FINAL V2]',e);}});}
 client.on('interactionCreate',async i=>{try{if(i.isStringSelectMenu()&&i.customId==='lc_help_final_v2')return i.update(help(i.guild,i.values[0]));if(i.isButton()&&i.customId==='lc_help_final_v2_home')return i.update(help(i.guild,'home'));}catch(e){console.error('[HELP FINAL V2 INTERACTION]',e);}});
};
