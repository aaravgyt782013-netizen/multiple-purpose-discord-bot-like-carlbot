...
module.exports=function attachEconomyPlus(client,prefix='.'){
 ...
 client.on('messageCreate',async m=>{
  try{
   ...
   if(sub==='buy'){
    const itemId=(p[0]||'').toLowerCase(),item=shops[itemId],n=Math.max(1,Math.min(99,Number(p[1])||1));
    if(!item)return m.reply(`❌ Item not found. Use ${P}eco shop.`);
    const total=item.price*n;if(me.balance<total)return m.reply(`❌ You need **${total.toLocaleString()}** coins.`);
    me.balance-=total;me.inventory[itemId]=(me.inventory[itemId]||0)+n;save(db);
    return m.reply(`🛒 Bought **${n}× ${item.name}** for **${total.toLocaleString()}** coins.`);
   }
   if(sub==='inventory'||sub==='inv'){
    const rows=Object.entries(me.inventory).filter(([,n])=>Number(n)>0).map(([id,n])=>`${shops[id]?.emoji||'📦'} **${shops[id]?.name||id}** × ${n}`);return m.reply({embeds:[embed('🎒 Inventory',rows.length?rows.join('\n'):'Your inventory is empty.')]});
   }
