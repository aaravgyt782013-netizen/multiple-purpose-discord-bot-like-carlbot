...
EXPECTED = {
    ...
    "automod": "Manage Server", "setlog": "Manage Server",
    ...
    "ticketsetup": "Manage Channels", "ticketpanel": "Manage Channels", "setticketcategory": "Manage Server", "setticketlog": "Manage Server", "close": "Manage Channels", "ticketcategory": "Manage Channels",
...
def audit_file(path):
    ...
    for node in ast.walk(tree):
        ...
        for dec in node.decorator_list:
            if not isinstance(dec, ast.Call) or not isinstance(dec.func, ast.Attribute):
                continue
            if dec.func.attr in {"hybrid_group", "group"}:
                name = kw_string(dec, "name") or node.name
                group_names[node.name] = name
                break
        nodes.append(node)
    ...
    for node in nodes:
        ...
        for dec in node.decorator_list:
            if isinstance(dec, ast.Call) and isinstance(dec.func, ast.Attribute):
                ...
                if attr in {"hybrid_command", "command", "hybrid_group", "group"}:
                    ...
                    if attr in {"hybrid_group", "group"}:
                        command_name = explicit or node.name
                    elif isinstance(dec.func.value, ast.Name) and dec.func.value.id in group_names:
                        parent = group_names[dec.func.value.id]
                        command_name = explicit or node.name
                    elif dec.func.value.__class__.__name__ == "Name" and dec.func.value.id == "commands":
                        command_name = explicit or node.name
                    elif attr == "hybrid_command" and isinstance(dec.func.value, ast.Name) and dec.func.value.id == "commands":
                        command_name = explicit or node.name
        ...
        if command_name:
            qualified = f"{parent} {command_name}" if parent else command_name
            found[qualified] = ", ".join(permissions) if permissions else "None"
    return found
...
def main():
    ...
    for path in sorted(Path("cogs").glob("*.py")):
        if path.name == "__init__.py":
            continue
        found.update(audit_file(path))

    errors = []
    for command, expected in EXPECTED.items():
        actual = found.get(command)
        if actual != expected:
            errors.append(f"{command}: expected {expected!r}, found {actual!r}")
    for command, actual in found.items():
        if actual != "None" and command not in EXPECTED:
            errors.append(f"{command}: has permission check {actual!r} but is missing from EXPECTED")

    print(f"Permission audit: {len(found)} command definitions found.")
    for command in sorted(found):
        print(f"{command} -> {found[command]}")
    if errors:
        raise SystemExit("Permission audit FAILED:\n" + "\n".join(errors))
