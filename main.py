...
class LightCoreBot(commands.Bot):
    ...
    async def on_wavelink_node_ready(self,payload):
        ...
        log.info("Lavalink NODE READY event fired: node=%r uri=%r connected=%s",getattr(node,"identifier",LAVALINK_NAME),getattr(node,"uri",LAVALINK_URI),getattr(node,"is_connected",None))

    async def on_wavelink_node_closed(self,payload):
        node=getattr(payload,"node",None)
        log.warning("Lavalink NODE CLOSED event fired: node=%r reason=%r code=%r",getattr(node,"identifier",LAVALINK_NAME),getattr(payload,"reason",None),getattr(payload,"code",None))
...
bot.started_at=time.monotonic();bot.started_at_wall=discord.utils.utcnow()
COGS=["moderation","automod","logging","leveling","tickets","roles","currency","music","embeds","panels","welcome","giveaways","giveaway_plus","custom_commands","temp_voice","fun","games","serverinfo","admin","memberstats","applications","activitystats","status","utility","core"]
...
async def raw_lavalink_connectivity_check():
    parsed=urlparse(LAVALINK_URI);host=parsed.hostname;port=parsed.port or (443 if parsed.scheme=="https" else 80)
    if not host:
        log.error("Lavalink RAW CONNECTIVITY: could not parse host from LAVALINK_URI=%r",LAVALINK_URI);return False
    started=time.monotonic()
    try:
        reader,writer=await asyncio.wait_for(asyncio.open_connection(host,port),timeout=10)
        log.info("Lavalink RAW TCP CONNECTIVITY OK: host=%r port=%d elapsed=%.2fs",host,port,time.monotonic()-started)
...
async def connect_lavalink(client):
    ...
    if not uri_ok or not password_ok or not name_ok:
        log.error("Lavalink startup connection skipped: one or more required env vars are empty.");return
    ...
    for attempt,delay in enumerate((2,5,10),1):
        try:
            ...
            log.info("Lavalink connection attempt %d/3: awaiting wavelink.Pool.connect() with 15s hard timeout...",attempt)
            started=time.monotonic()
            await asyncio.wait_for(wavelink.Pool.connect(nodes=[node],client=client),timeout=15)
            log.info("Lavalink Pool.connect() RETURNED on attempt %d/3 after %.2fs; node=%r uri=%r",attempt,time.monotonic()-started,LAVALINK_NAME,LAVALINK_URI)
            return
        except asyncio.TimeoutError:
            log.error("Lavalink connection TIMED OUT after 15s on attempt %d/3",attempt)
        except Exception as exc:
            log.exception("Lavalink connection attempt %d/3 failed with %s: %s",attempt,type(exc).__name__,exc)
        if attempt<3:
            log.warning("Retrying Lavalink connection in %ss...",delay);await asyncio.sleep(delay)
...
async def on_ready():
    ...
    if ffmpeg:log.info("FFmpeg detected in PATH: %s (local FFmpeg is not used for Lavalink playback).",ffmpeg)
    else:log.warning("FFmpeg is not in PATH; Lavalink playback can still work.")
    if getattr(bot,"_slash_synced",False):return
    try:
        synced=await bot.tree.sync();bot._slash_synced=True;log.info("Synced %d slash commands",len(synced))
...
async def on_command_error(ctx,error):
    ...
    if isinstance(error,commands.MissingPermissions):
        names=", ".join(p.replace("_"," ").title() for p in error.missing_permissions);return await ctx.send(embed=discord.Embed(title="🚫 Permission Required",description=f"You need **{names}** permission to use this command.",color=discord.Color.red()))
    if isinstance(error,commands.BotMissingPermissions):
        names=", ".join(p.replace("_"," ").title() for p in error.missing_permissions);return await ctx.send(embed=discord.Embed(title="⚠️ Bot Permission Missing",description=f"LightCore needs **{names}** permission to complete this action.",color=discord.Color.orange()))
...
async def runner():
    init_db();validate_command_names();log.info("Startup validation passed; connecting to Discord.");await bot.start(BOT_TOKEN)

if __name__=="__main__":
    start_health_server();asyncio.run(runner())
